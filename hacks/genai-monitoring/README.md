...
1. Navigate to the Genkit monitoring dashboard for your deployed application.
1. What are the key aggregate stability metricsd for the entire project. What they indicate about the overall health of your application.
1. Locate the list of individual GenAI **features** in your app and identify the types of performance indicators (metrics) shown for each.
1. Find the specific feature that handles the core user interactions with the app and investigate its aggregate metrics, including any occurrences of failures.
1. Examine an individual execution (a trace) of a user interaction to identify and understand the sequence of steps that occurred and the specific details recorded for each step.
1. Compare traces generated by different types of user queries (e.g., a question requiring a movie recommendation versus a simple greeting) and identify notable differences in their execution paths or details.
